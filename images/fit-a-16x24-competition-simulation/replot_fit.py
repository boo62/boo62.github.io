...
if guess_no == -1:
    data_file = "sim_fits/{0}x{1}_comp_model/uniform_guess/stop_factr_10e{2}.json"
    data_file = data_file.format(rows, cols, factr_pow)
else:
...
# ms: markersize, mew: markeredgewidth, lw: linewidth
plotter.plot_est(plate, plate.comp_est,
                 title='Uniform initial guess (factr=10E{0})'.format(factr_pow),
                 sim=True, legend=True, ms=10.0, mew=2.0, lw=2.0)
